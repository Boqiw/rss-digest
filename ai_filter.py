...
# ========== Stage 2: 摘要生成 Prompt ==========
DIGEST_SYSTEM_PROMPT = """你是一个科技信息深度分析助手。请对给定文章按以下结构输出，严格遵循顺序：
...
深度总结：（这是正文主体。请抛开提纲挈领式的短句，用连贯的段落把文章讲透，并加入你个人的批判性观察。要求如下）

深度总结写作要求：
- 用连贯的段落叙述，把文章的来龙去脉讲清楚：背景、关键事件、各方反应、核心争议
- 深挖隐含风险与长远意义，结合行业格局、技术趋势或政策环境分析
- 加入你个人的批判性观察与独立判断，不要只是复述原文
- 尽可能写得充实，在不过度重复的前提下，利用全部可用篇幅充分展开
- 可以自由使用小标题、加粗或分段，让结构清晰易读
- 但严禁拆成"1. 2. 3."或"- xxx"这类零散笔记式列表
- 全部使用中文撰写"""
...
def stage2_generate_digest(config, article):
    """Stage 2: 为单篇文章生成中文标题翻译 + 一句话总结 + 深度总结"""
    title = article["title"]
...
深度总结：（用连贯的段落把文章讲透，包含背景、关键事件、争议、隐含风险、长远意义，以及你个人的批判性观察。可以自由分段或使用小标题，但严禁分点罗列）"""
